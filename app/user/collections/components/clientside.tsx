  ...
  return (
    <>
      ...
      <div className="grid grid-cols-5 gap-2 sm:gap-4 w-full">
        {[...Array(10)].map((im, idx)=><label key={useId()} id={`/dicebear/avataaarsNeutral-${idx}.svg`} className={`overflow-hidden custom-outline cursor-pointer border-blue-600 border-4 border-opacity-30 bg-neutral-950 aspect-square relative ${ selected===`/dicebear/avataaarsNeutral-${idx}.svg` ? 'border-opacity-80' : 'hover:border-opacity-80' } rounded-lg`} tabIndex={0} onKeyUp={handleChangeLabel}>
      ...
      <div className="grid grid-cols-5 gap-2 sm:gap-4 w-full">
        {[...Array(10)].map((im, idx)=><label key={useId()} id={`/dicebear/thumbs-${idx}.svg`} className={`overflow-hidden custom-outline cursor-pointer border-blue-600 border-4 border-opacity-30 bg-neutral-950 aspect-square relative ${ selected===`/dicebear/thumbs-${idx}.svg` ? 'border-opacity-80' : 'hover:border-opacity-80' } rounded-lg`} tabIndex={0} onKeyUp={handleChangeLabel}>
